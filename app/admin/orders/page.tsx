  ...
  return (
    ...
    >
      <div className="max-w-4xl mx-auto py-4 px-2 sm:px-4 space-y-4 font-sans">
        ...
        {/* Search Bar */}
        <form method="GET" action="/admin/orders" className="bg-white rounded-2xl border border-gray-100 shadow-sm p-3 flex gap-2">
          ...
          {searchQuery && (
            <Link
              href={`/admin/orders${statusFilter ? `?status=${statusFilter}` : ''}`}
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-none">
          <Link
            href={`/admin/orders${searchQuery ? `?search=${encodeURIComponent(searchQuery)}` : ''}`}
          ...
          <Link
            href={`/admin/orders?status=pending${searchQuery ? `&search=${encodeURIComponent(searchQuery)}` : ''}`}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all shrink-0 ${
          ...
          <Link
            href={`/admin/orders?status=in_progress${searchQuery ? `&search=${encodeURIComponent(searchQuery)}` : ''}`}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all shrink-0 ${
          ...
          <Link
            href={`/admin/orders?status=delivered${searchQuery ? `&search=${encodeURIComponent(searchQuery)}` : ''}`}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all shrink-0 ${
          ...
          <Link
            href={`/admin/orders?status=cancelled${searchQuery ? `&search=${encodeURIComponent(searchQuery)}` : ''}`}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all shrink-0 ${
