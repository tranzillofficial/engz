...
export default function LoginPage() {
  ...
  // Controlled states to preserve user input on error
  ...
  return (
    <div className="auth-card animate-fade-in font-sans">
      <div className="auth-card-header">
        <span className="inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-orange-50 dark:bg-orange-950/40 text-[#FA3802] text-xl mb-3">
          ↗
        </span>
      ...
      <div className="auth-divider">
        <span>أو البريد الإلكتروني</span>
      </div>
        ...
        <button type="submit" disabled={isPending} className="btn-primary btn-full">
          {isPending ? (
            <span className="btn-loading">
              <span className="spinner" aria-hidden="true" /> جاري تسجيل الدخول...
            </span>
          ) : (
            'تسجيل الدخول'
          )}
      ...
      <div className="flex items-center justify-center gap-2 text-xs text-slate-500">
        ...
        <Link href="/register" className="font-black text-[#FA3802] hover:underline">
          إنشاء حساب جديد
        </Link>
      </div>

      <div className="auth-divider">
        <span>أو بدون حساب</span>
      </div>
      <Link href="/orders/new" className="btn-outline btn-full text-center">
        🚀 اطلب توصيل فوري كزائر
      </Link>
      <Link
        href="/join-driver"
        className="text-center text-xs text-[#FA3802] font-bold hover:underline py-1"
      >
