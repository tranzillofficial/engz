...
import { agentReviewPaymentAction } from '@/lib/actions/agent';
import { Card, Button, PaymentStatusBadge, Alert } from '@/components';

interface AgentPaymentsClientProps {
  payments: any[];
}

export function AgentPaymentsClient({ payments }: AgentPaymentsClientProps) {
  ...
  const handleReview = (paymentId: string, action: 'confirmed' | 'rejected') => {
    ...
    startTransition(async () => {
      const res = await agentReviewPaymentAction(paymentId, action);
      if (res?.error) {
      ...
      {payments.length === 0 ? (
        <div className="p-8 text-center bg-gray-50 dark:bg-gray-800/40 rounded-2xl">
          ...
          <p className="text-xs text-gray-500 mt-1">
            عندما يسجل أي طيار إشعار تحويل لمستحقاته سيظهر هنا لمراجعته وتأكيد استلامه.
          </p>
        </div>
      ) : (
        <div className="space-y-3">
                ...
                <div className="bg-gray-50 dark:bg-gray-800/50 p-2.5 rounded-xl text-xs space-y-1">
                  ...
                  {p.notes && (
                    <div className="text-[11px] text-gray-600 dark:text-gray-400 pt-1">
                      <span className="font-semibold">ملاحظات الطيار:</span> {p.notes}
                    </div>
                  )}
                </div>

                {/* Current Driver Balance Snapshot */}
